...
def load_response_matrix(material: str, q: str, mass: str) -> np.ndarray:
    path = DATA_DIR / "curlyRplotting" / f"{material}_R5_q{q}M{mass}_Ko_thresholded_refined_exposure.csv"
    return np.genfromtxt(path, delimiter=",")


def load_ratebin(eta: str, material: str, q: str, mass: str) -> np.ndarray:
    path = DATA_DIR / "Ratebin" / f"Event5Eta{eta}Mat{material}q{q}M{mass}.csv"
    return np.genfromtxt(path, delimiter=",")
