...
(*                                                                            *)
(*  Per-bin peak-value cut, computed on the SMOOTH response R(v_min) BEFORE      *)
(*  integration: keep the window [a, b] between the OUTERMOST v_min where R is    *)
(*  >= a fraction `frac` of the peak value (default 0.2 -> cut the outer tails    *)
(*  below 20% of the peak). An interior dip below the threshold (the valley of a  *)
(*  double-peaked response) is kept, so neither peak is removed. The cut points   *)
(*  are a property of the response, not of the v_min binning, so they are found   *)
(*  on a fine grid. The matrix is then trimmed to the union of the per-bin windows. *)
(*                                                                            *)
...
(*    [frac]   peak-value fraction setting the cut: keep the window [a, b]      *)
(*             around the peak where R >= frac * peak, on both sides            *)
(*             (default 0.2). frac = 0 disables the cut.                        *)
(*                                                                            *)
(*  e.g.  wolframscript -file 10_response_matrix.wl ALL 1 800 1000 0.2          *)
(*        wolframscript -file 10_response_matrix.wl Al_q0M3_R5 1 800 1000 0.2   *)
(*                                                                            *)
...
(* Per-bin peak-value cut: keep the contiguous window [a, b] around each bin's
   peak where the response stays >= fraction `peakFrac` of its peak value,
   zeroing both the low-v_min rise and the high-v_min tail below that level
   (default 0.2 -> cut where R has fallen to 20% of its peak). Localising the
   response this way lets the monotone inverse recover eta to the window edge.
   peakFrac = 0 disables the cut. *)
...
buildMatrix[wdxBaseName_] := Module[
  ...
  (* ---- per-bin peak-value window on the SMOOTH response (before integration) ----
     The window [a, b] = the OUTERMOST crossings of peakFrac * peak: a is the
     first fine-grid v_min where R >= peakFrac * peak and b the last. It is a
     property of the smooth response R(v_min), NOT of the v_min binning, so it is
     found here on a fine grid (independent of the matrix's N). Only the outer
     sub-threshold tails are cut; an interior dip below the threshold (the valley
     of a double-peaked response) is kept, so neither peak is removed.
     peakFrac = 0 keeps the full domain. *)
  nFine  = 4000;
  ...
  windowEdges[f_] := Module[{rs, thr, idx},
    ...
    {fineV[[First[idx]]], fineV[[Last[idx]]]}
  ];
  ...
  (* M[i, j] = integral of bin-i response over interval j, clipped to the bin's
     peak-value window [a, b] = winEdges[[i]]: keep [a, b] and zero both the
     low-v_min rise and the high-v_min tail below peakFrac * peak. kps converts
     dv to natural. *)
  matrix   = kps Table[
