...
ALPHA=0.30; mass="3"
GEV=1e9/abs(eta_halo_at(GEV_NATIVE,"1",np.genfromtxt(CSV_DIR/"Al_q0M1_R5.csv",delimiter=",",names=True)["vmin"])).max()
d=np.genfromtxt(CSV_DIR/f"Al_q0M{mass}_R5.csv",delimiter=",",names=True); v=d["vmin"]
...
cn=np.linalg.norm(mp,axis=0); C=CM_at(GEV)
...
ax.set_title("TES Al q0 M3 R5 SHM, alpha=0.3  (uniform vs colnorm)")
ax.grid(True,which="both",ls="--",alpha=0.35); ax.legend(fontsize=10)
fig.savefig("tes_M3_a030.pdf",bbox_inches="tight"); print("saved tes_M3_a030.pdf")
